namespace Hion {
	...
	export declare interface Position2D {
		x, y: number;
	}
	export declare interface PointPosition {
		x, y: number;
		next, prev: PointPosition;
	}
	...
	export class Point {
		...
		constructor (public parent: SdkElement, public name: string, public type: number) {
		...
		createPath() {
			...
			var v1 = {x: point1.pos.x, y: point1.pos.y, side: 0, l: 0};
			...
			var v2 = {x: point2.pos.x, y: point2.pos.y, side: 0, l: 0};
	...
	function tracePath(p: PointPosition, v1, v2) {
		if(v1.side != v2.side) {
			if(v1.x == v2.x) {
